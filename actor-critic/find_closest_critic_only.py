...
for episode in range(HM_EPISODES):
    ...
    for i in range(SIZE*10):
        ...
        left = np.array([1., 0.])
        right = np.array([0., 1.])
        activations = model.predict_on_batch(np.vstack((np.concatenate(([obs], left)), np.concatenate(([obs], right)))))
    ...
    # ipdb.set_trace()
    x = np.hstack((states.reshape(-1,1), np.array(actions.reshape(-1,2))))
    x = tf.expand_dims(x, 0)
    model.train_on_batch(x, Y)
    # ipdb.set_trace()
    for i, cell in enumerate(env[2]):
        # ipdb.set_trace()
        activations = model.predict_on_batch(np.vstack((np.concatenate(([i/SIZE], left)), np.concatenate(([i/SIZE], right)))))
        probs = tf.nn.softmax(tf.squeeze(np.array(activations)))
