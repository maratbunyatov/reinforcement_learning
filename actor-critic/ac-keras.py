...
while True:  # Run until solved
    ...
    with tf.GradientTape() as tape:
        for timestep in range(1, max_steps_per_episode):
            ...
            state, reward, done, _ = env.step(action)            
                ...
                or state[2] < -env.theta_threshold_radians
                or state[2] > env.theta_threshold_radians
            )
    ...
    if episode_count % episode_count == 0:
        print(f"episode: {episode_count}, episode_reward: {episode_reward}, running reward: {running_reward:.2f}")

    if running_reward > 1495:  # Condition to consider the task solved
